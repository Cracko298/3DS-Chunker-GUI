import sys, shutil, re, random, json, asyncio, logging
...
def convert(
    ...
    if world_out.exists():
        if not world_out.is_dir() or not (world_out / "level.dat").is_file():
            ...
            print("World with same name found.")
            while True:
                choice = messagebox.askyesno("WARNING", f"A World with the name '{world.name}' already exists.\nWould you like to Overwrite the World?\nIf you select 'NO', no conversion will happen.")
                if choice:
                    shutil.rmtree(world_out)
                    break
                else:
