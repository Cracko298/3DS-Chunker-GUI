import sys, time, importlib.resources, shutil, json
...
def main(
    ...
    blank_world_path = ".\\3dschunker\\data\\blankworld"
    ...
    if mode == "convert":
        ...
        answer = messagebox.askyesno("Installation Notice", "If you're using Windows.\nThe GUI can install the World for you.\nMay the GUI install the World to Minecraft?")
        if answer:
            dst = os.path.expandvars(f"%APPDATA%\\.minecraft\\saves\\{world.name}")
            shutil.copytree(f".\\{world.name}", dst, dirs_exist_ok=True)
            messagebox.showinfo("Notice", "World has been Converted and Installed to Java Edition\nUse any version from v1.16.5-v1.18.X to Load it.")
        else:
            messagebox.showinfo("Notice", "World has been Converted to Java Edition v1.16.X\nUse any version from v1.16.5-v1.18.X to Load it.\nYou will need to install it Manually.")
    elif mode == "extract":
